...
module.exports = {
    ...
    buildBargainFinderMax : function(airAvailRQ, blockedAirlines , callback) {
        ...
        var originDestinationInformation = buildOriginDestinationInformation(airAvailRQ);
        var travelPreferences = buildTravelPreferences(blockedAirlines, airAvailRQ);
        var tpaExtension = buildTPAExtension();
        buildTravelerInfoSummary(airAvailRQ);
...
function buildOriginDestinationInformation(airAvailRQ){
...
function buildTravelPreferences(blockedAirlines, airAvailRQ){
    ...
    blockedAirlines['blocked_airlines'].forEach(function(airline, index){
        var item = {
            "Code" : airline.code
        }
...
function buildTravelerInfoSummary(airAvailRQ){

    var isPublicFare = false;
    var isPrivateFare = false;

    if(airAvailRQ.faretype == fareTypeSearchConstants.PUBLIC_SEARCH){
        isPublicFare = true;
    } else {
        isPrivateFare = true;
    }
    ...
    if(numberOfADT > 0){
        if(airAvailRQ.fareTypeSearch == fareTypeSearchConstants.PUBLIC_SEARCH){
            var  passengerADT = {"PassengerTypeQuantity": [{"Code": "ADT","Quantity": numberOfADT}]};
            array.push(passengerADT);
            ...
            if(airAvailRQ.negotiation != null){
                airAvailRQ.negotiation.forEach(function(negotiation){
                    var  passengerPri = {
                        "PassengerTypeQuantity": [{"Code": negotiation,"Quantity": numberOfADT}]
                    };
                    array.push(passengerPri);
                });
            }
        }
    }
    ...
    if (numberOfCNN > 0) {
        var  passengerCNN = {"PassengerTypeQuantity": [{"Code": "CNN","Quantity": numberOfCNN}]};
        array.push(passengerCNN);
    }

    if (numberOfINF > 0) {
        var  passengerINF = {"PassengerTypeQuantity": [{"Code": "INF","Quantity": numberOfINF}]};
        array.push(passengerCNN);
    }
    ...
    logger.log('info', travelerInfoSummary);
    return travelerInfoSummary;
}

function buildTPAExtension(){
